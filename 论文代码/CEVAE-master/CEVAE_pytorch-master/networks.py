...
class p_x_z(nn.Module):
    ...
    def __init__(self, dim_in=20, nh=3, dim_h=20,dim_out_con=6):
        super().__init__()
        # save required vars
        self.nh = nh
...
class q_z_tyx(nn.Module):
    ...
    def forward(self, xy, t):
        ...
        scale = (1-t)*sigma_t0 + t*sigma_t1
        scale = scale.clamp(min=1e-5)  # 避免出现0或极小的正数
        # Set mu and sigma according to t
        z = normal.Normal((1-t)*mu_t0 + t * mu_t1, scale)
        return z
